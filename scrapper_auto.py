...
def guardar_excel_con_autoajuste(dataframe):
    """Guarda el Excel automaticamente en la carpeta capturas/ con la fecha de hoy."""
    try:
        from openpyxl import load_workbook
        from openpyxl.styles import PatternFill, Font, Alignment, Border, Side
        from openpyxl.utils import get_column_letter

        # --- GUARDADO AUTOMATICO en capturas/ ---
        carpeta = "capturas"
        os.makedirs(carpeta, exist_ok=True)
        fecha_str = datetime.now().strftime("%Y-%m-%d")
        mitad = "AM" if datetime.now().hour < 12 else "PM"
        ruta_archivo = os.path.join(carpeta, f"Chileautos {fecha_str} {mitad}.xlsx")
        # Nada se pisa ni se mueve: AM y PM conviven en capturas/. Si hubiera
        # una tercera corrida en la misma mitad del dia, se agrega la hora.
        if os.path.exists(ruta_archivo):
            hora = datetime.now().strftime("%H-%M")
            ruta_archivo = os.path.join(carpeta, f"Chileautos {fecha_str} {mitad} ({hora}).xlsx")

        # Guardar base con pandas
        dataframe.to_excel(ruta_archivo, index=False)
        print(Fore.CYAN + f"[scraper] Base guardada en: {ruta_archivo}" + Style.RESET_ALL)
        ...
        row_fill_par = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type="solid")
        row_fill_impar = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid")
        ...
        for cell in ws[1]:
            ...
            if str(cell.value).lower() in ["url_detalles", "enlace web"]:
        ...
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
            ...
            for cell in row:
                ...
                if url_col_idx and cell.column == url_col_idx and cell.value:
                    link = str(cell.value)
                    cell.hyperlink = link
                    cell.value = "Abrir Publicación 🔗"
                    cell.font = link_font
                    cell.alignment = center_alignment
                if price_col_idx and cell.column == price_col_idx and cell.value is not None:
                    try:
                        ...
                        pass
                if kms_col_idx and cell.column == kms_col_idx and cell.value is not None:
                    try:
                        val = int(str(cell.value).replace(".", "").replace(",", ""))
                        cell.value = val
        ...
        for col in ws.columns:
            ...
            if url_col_idx and column == get_column_letter(url_col_idx):
                adjusted_width = 22
        ...
        print(Fore.GREEN + f"✅ Excel guardado con estilos en: {ruta_archivo}" + Style.RESET_ALL)

    except Exception as e:
        # Si falla el estilo, el archivo base ya fue guardado igual
        print(Fore.YELLOW + f"⚠️ Estilos no aplicados ({e}), pero el archivo base fue guardado." + Style.RESET_ALL)
...
if __name__ == "__main__":
    ...
    print(Fore.CYAN + "Al finalizar, el archivo se guardará automáticamente en la carpeta capturas/." + Style.RESET_ALL)
    time.sleep(7)
